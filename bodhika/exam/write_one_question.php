...
/* ── Access control ────────────────────────────────────────────────────
   ...
   We check the assignment status first (to catch already-completed exams),
   then use Enrollment::canAccess() as the single authoritative gate for
   both paths.  Never redirect solely because exam_assignments has no row.
──────────────────────────────────────────────────────────────────────── */
if (!$isAdmin && $myUid) {

    /* 1. Check assignment status (if the table exists) */
    $asgRow = null;
    try {
        ...
        /* exam_assignments table not yet created — skip */
    }

    if ($asgRow && $asgRow['Status'] === 'Completed') {
        /* Already submitted — redirect to history */
        header('Location: history.php?InfoId=' . $examId);
        exit;
    }

    /* 2. Enrollment / payment gate — covers assigned and self-enrolled exams.
    ...
    /* 3. Attempt-limit gate (migration_v36) — exam MaxAttempts, overridable
          per student via exam_attempt_overrides. 0 = unlimited. */
...
$pageHead  = '<style>
  ...
  .qnav-arrow { width:42px;height:42px;border-radius:50%;border:none;background:#312e81;color:#fff;
                font-size:1.2rem;display:flex;align-items:center;justify-content:center;
                cursor:pointer;flex-shrink:0;transition:.15s; }
  .qnav-arrow:hover:not(:disabled) { background:#4338ca; }
  .qnav-arrow:disabled { background:#cbd5e1;cursor:not-allowed;opacity:.7; }
  ...
  .ei-dismiss     { margin-left:auto;background:none;border:1px solid #a5b4fc;color:#4338ca;padding:5px 12px;
...
<form id="frmWriteExam" method="post" action="submit.php" onsubmit="return validateExam(event)">
  ...
  <div class="exam-instructions-banner" id="examInstructionsBanner">
    <div class="ei-header">
      ...
      <div>
        ...
        <div class="ei-sub">Please read the following details carefully before you begin.</div>
      </div>
      <button type="button" class="ei-dismiss"
              onclick="document.getElementById('examInstructionsBanner').style.display='none'">
        Got it &times;
      </button>
    </div>
    <div class="ei-grid">
  ...
  <div class="card">
    ...
    <div style="padding:14px;">
      ...
      <!-- ── One-question-at-a-time navigator ─────────────────────────────
           Prev/Next/Skip move between questions; the palette below jumps
           straight to any question by number and color-codes it (current /
           answered / skipped / not yet visited) so a skipped question is
           easy to find and come back to before submitting. -->
      <div class="qnav-bar">
        <div class="qnav-side">
          <button type="button" class="qnav-arrow qnav-prev" onclick="qPrev()" title="Previous question">&#8249;</button>
          <span class="qnav-count"></span>
        </div>
        <div class="qnav-side">
          <button type="button" class="qnav-skip" onclick="qSkip()">&#8942; Skip for now</button>
          <button type="button" class="qnav-arrow qnav-next" onclick="qNext()" title="Next question">&#8250;</button>
        </div>
      </div>
      ...
      <div class="q-card" id="qrow<?php echo $qIdx; ?>"
        ...
        <?php if ($qType === 'YESNO'): ?>
          <?php renderQuestionPrompt($q, 'yesno-instr'); ?>
        ...
        <?php elseif ($qType === 'MATCH'): ?>
          <?php
            renderQuestionPrompt($q, 'yesno-instr');
        ...
        <?php elseif ($qType === 'MULTI'): ?>
          <?php
            $expCount = (int)($q['ExpectedAnswerCount'] ?? 0);
        ...
        <?php elseif ($qType === 'DROPDOWN'): ?>
          <?php
            // Show image above the dropdown if one is attached
